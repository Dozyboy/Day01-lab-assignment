...
def call_openai(
    ...
    client = OpenAI(api_key=_get_openai_api_key(), base_url=OPENAI_BASE_URL)
...
def retry_with_backoff(
    ...
    if last_error is not None:
        raise last_error
    raise RuntimeError("retry_with_backoff failed unexpectedly")
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def format_row(row: list[str]) -> str:
        return " | ".join(cell.ljust(column_widths[index]) for index, cell in enumerate(row))
...
# Expose a stable module alias so unit tests can patch symbols reliably.
sys.modules.setdefault("template", sys.modules[__name__])
call_openai.__module__ = "template"
call_openai_mini.__module__ = "template"
compare_models.__module__ = "template"
streaming_chatbot.__module__ = "template"
retry_with_backoff.__module__ = "template"
batch_compare.__module__ = "template"
format_comparison_table.__module__ = "template"
